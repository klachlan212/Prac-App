...
- **Supabase:** project ref `kkytglabcwevvnjmmrhy` (`prac-app-database`, region `ap-southeast-2` Sydney). 10 tables RLS-enabled; 11 universities + 7 RN ANSAT standards seeded. Demo user `klachlan212@gmail.com` / `PracDemo2026!` with seeded profile + placement + 4 reflections. Security advisors pending: mutable `search_path` on 3 SECURITY DEFINER fns; `organizations` RLS-enabled-no-policy.
