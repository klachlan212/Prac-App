...
- **Supabase:** project ref `kkytglabcwevvnjmmrhy` (`prac-app-database`, region `ap-southeast-2` Sydney). RLS-enabled tables incl. the core set + the hospital directory (`hospitals`, `hospital_tips`, `hospital_reference_cards`, `hospital_tip_votes`); `profiles.is_moderator` flags global moderators. 11 universities + 7 RN ANSAT standards seeded; hospital directory seeded (6 VIC hospitals, 12 published tips). Demo user `klachlan212@gmail.com` / `PracDemo2026!` (now a moderator) with seeded profile + placement + reflections. Security advisors pending: mutable `search_path` on the **pre-0004** SECURITY DEFINER fns (the 0004 fns pin it); `organizations` RLS-enabled-no-policy; `hospital_tip_votes` RLS-enabled-no-policy is **intentional** (deny-all, definer-RPC-only); `submit_hospital_tip`/`cast_hospital_vote` anon-executable is **intentional** (the anonymous-write feature).
