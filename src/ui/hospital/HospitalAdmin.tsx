...
import { CATEGORIES, type Hospital, type ReferenceCard, type TipCategory } from '@/src/content/hospitals'
import {
  deleteRefCard,
  fetchHospitals,
  fetchRefCards,
...
const BLANK: HospitalInput = { id: '', name: '', location: '', region: '', intro: '', curatedBy: '' }
    ...
    <div>
      ...
      <Card className="space-y-3">
        ...
        <form onSubmit={saveHospital} className="space-y-3" noValidate>
          {!editingExisting && (
            <Field label="Slug (URL id)" hint="lowercase, hyphens — e.g. northern-hospital">
              <Input
                value={form.id}
          ...
          <Field label="Region" hint="Groups hospitals on the directory — e.g. Melbourne">
            <Input
              value={form.region}
      ...
      {cardsFor && <RefCardManager hospital={cardsFor} />}
    </div>
  )
}
